...
void PWM_INIT(PWMGEN *v){
	     ...
	     EPwm6Regs.CMPA.half.CMPA=1000;	// PWM 5A - Leg4											     			
		 ...
		 EPwm6Regs.AQCTLB.all = CAU_SET + CAD_CLEAR;																	
		 ...
		 EPwm6Regs.DBCTL.all = BP_DISABLE ;														
         /* Init Dead-Band Generator for EPWM1-EPWM3*/						
         EPwm1Regs.DBFED = DBCNT_INIT_STATE;	//2 usec Delay							
         EPwm1Regs.DBRED = DBCNT_INIT_STATE;	//2 usec Delay							
         EPwm2Regs.DBFED = DBCNT_INIT_STATE;	//2 usec Delay							
         EPwm2Regs.DBRED = DBCNT_INIT_STATE;	//2 usec Delay							
         EPwm3Regs.DBFED = DBCNT_INIT_STATE;	//2 usec Delay							
         EPwm3Regs.DBRED = DBCNT_INIT_STATE;	//2 usec Delay
         EPwm6Regs.DBFED = 0;	//No delay							
         EPwm6Regs.DBRED = 0;	//No delay						
        ...
        EDIS;                         /* Disable EALLOW*/			
	     ...
	     EPwm6Regs.TZFRC.bit.OST = 1 ; // Forces a one-shot trip event for EPWM4
	     EDIS;
...
void PWM_Drive(PWMGEN *v,float32 DutyR,float32 DutyS,float32 DutyT,float32 Duty_STS)
{																		
...
/*Compute the compare from the related duty cycle ratio*/						
	...
	Tmp = MPeriod*v->MfuncC6;				                            					
	v->PWM6out = (Uint16)(Tmp+MPeriod);	
	...
	EPwm6Regs.CMPA.half.CMPA=v->PWM6out;
}
