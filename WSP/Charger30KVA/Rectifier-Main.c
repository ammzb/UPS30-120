...
//----------------------------------------
{
    ...
    if(UPS.Rectifier.Status.On == 1 && Flag_Start==1){
    	Flag_Start=2;     //Spare
    }
	...
	if(UPS.Rectifier.Status.On == 1 && Flag_Start==3 && VTimer2[0] > 400 ){   //400 f0r 20 sec.
	...
	if(UPS.Rectifier.Status.On == 1 && Flag_Start==4 && VTimer2[0] > 420 && UPS.Rectifier.Status.On_K3==0 && V_DC > VDC_Min_Start){
		...
		if(UPS.Rectifier.Status.On == 1 && Flag_Start==5 && VTimer2[0] > 500 && UPS.Rectifier.Status.On_K2==1){
...
//======================================================================================================================
   ...
   if(Counter_Boost_Start == 220){	  	
    ...
    if(Counter_Boost_Start > 220 && Counter_Boost_Start < 225){
    	if(UPS.Rectifier.Status.On_K2==0){
	    	Delay_Mains_Contactor++;
	    	if(Delay_Mains_Contactor == 20 ){
	    		Counter_Boost_Start++;
	    		Delay_Mains_Contactor=0; 
	    	} 		
    	}
    	else
    		Counter_Boost_Start = 221;	
    }

    
    if(Counter_Boost_Start == 225){
    	Counter_Boost_Start++; 
    	GpioDataRegs.GPCSET.bit.GPIO76 = 1; //Turn on K3 Contctor	
    }
    
    if(Counter_Boost_Start > 225 && Counter_Boost_Start < 230){
    	if(UPS.Rectifier.Status.On_K3==1)
    		Counter_Boost_Start++; 	
    	else
    		Counter_Boost_Start =226;
    }
    ...
    if(Counter_Boost_Start==230){
		...
		EPwm3Regs.DBFED = DEADTIME_BALANCER;	//Max 20% duty cycle							
        EPwm3Regs.DBRED = DEADTIME_BALANCER;	//Max 20% duty Cycle
		
		Counter_Boost_Off=0;
		UPS.Balancer.Status.On = 1;
		
		Flag_Balancer_ClearTrip = 1;
 ...
 //----------------------------Turn off boost--------------------------------------------
    	if(Counter_Boost_Start == 231){
    		...
    		if(Counter_Boost_Off == 800){
    			...
    			EPwm3Regs.DBFED = DEADTIME_RECTIFIER;	//4 usec Delay
    			EPwm3Regs.DBRED = DEADTIME_RECTIFIER;	//4 usec Delay
    		...
    		if(Counter_Boost_Off > 1000 && Counter_Boost_Off < 1005){
    			if(UPS.Rectifier.Status.On_K3 == 0){
    				Delay_Null_Contactor++;
    				if(Delay_Null_Contactor == 20){
    					Counter_Boost_Off++;
    					Delay_Null_Contactor=0;
    				}
    			}
    			else
    				Counter_Boost_Off = 1001;
    		}
    		...
    		if(Counter_Boost_Off > 1005 && Counter_Boost_Off < 1010){
    			if(UPS.Rectifier.Status.On_K2 == 1)
    				Counter_Boost_Off++;
    			else
    				Counter_Boost_Off = 1006;
    		}

    		if(Counter_Boost_Off == 1010 && UPS.Rectifier.Status.On_K2==1){
    			Flag_Start = 17;
    			Counter_Boost_Off++;
    		}
    	}
...
//	CNTL_2P2Z_F_FUNC( &Coeff3_h2,&Vars3_h2); 
    ...
    if(Counter_Boost_Start > 230 && UPS.Boost.Status.On == 1 && UPS.Rectifier.Status.On == 0 && UPS.Balancer.Status.On == 1){
    	DutyT=cntl_pi9.Out;   
    	if(DutyT > 0.5)
			DutyT=0.5; 	
		if(DutyT <- 0.5)
...
void Protection(void){
    ...
    if(GpioDataRegs.GPCDAT.bit.GPIO68==0)
        ...
        UPS.Rectifier.Status.On_K2=0;	//k2 bypass softstart
...
//-----------------------------------------------------Boost Faults-----------------------------------------------------------------		
	if(UPS.Boost.Status.On==1){
		...
		if(V_DC_BAT < deep_dischage_voltage)
...
//===========================================================================================================================================
//-----------------------------------------------------Balancer Faults-----------------------------------------------------------------	
	if(UPS.Balancer.Status.On == 1){
		if((I_T > RST_Rectifier_ShortCircuit) || (I_T < -RST_Rectifier_ShortCircuit))
			UPS.Balancer.Status.ShortCircuit=1;
		else
