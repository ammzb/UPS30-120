...
Uint16 Counter_Test = 0;
float32 Log_Test = 0.0;

Uint16 Flag_Start=0,Counter_Start=0,Flag_Sync=2,Flag_ShutDown=0,Counter_Emergency_Shutdown=0,nsamples=0,Counter_SyncSlowly=0,Flag_PowerOk=0;
...
Uint16 PowerGood=0, PowerBad=0, NormalPower_Counter=0, TurnOnBypass=0,Flag_ZCD=0,Counter_Sequence_OK=0, Counter_Sync_OK=0, GPIO58_prev=0,Flag_Manual_STS = 0, Flag_Manual_Bypass = 0, Flag_Auto_STS = 0;
	...
	if(sine_mainsV_R.nsamples == 0){
		if(UPS.Inverter.Status.BypassFail == 0 && UPS.Inverter.Status.PG == 1 && Flag_ShutDown == 0){// from GPIO59 says input voltage is in range 
			if((fabs(spll1.theta[0] - spll2.theta[0]) < MAX_PHASEDIFF) || (fabs(spll1.theta[0] - spll2.theta[0]) > (2.0*3.1415926 - MAX_PHASEDIFF))){
			    Counter_Sync_OK++;
			    Log_Test = spll1.theta[0] - spll2.theta[0];
			}
...
//---------------------PI Regulator for Phase a Current--------------------------------------- 
		cntl_pi4.Ref =cntl_pi1.Out + 1000.0*(Vars1_h1.Out + K*0.5*Vars1_h3.Out + K*0.5*Vars1_h5.Out + K*0.2*Vars1_h7.Out + K*0.2*Vars1_h9.Out + K*0.0*Vars1_h11.Out);
		//cntl_pi4.Ref =cntl_pi1.Out + 3800.0*Vars1_h1.Out;
	   	cntl_pi4.Fbk = I_R; 
	    CNTL_PI_F_FUNC(&cntl_pi4);
    
//---------------------PI Regulator for Phase b Current-------------------------------------- 
		cntl_pi5.Ref =cntl_pi2.Out + 1000.0*(Vars2_h1.Out + K*0.5*Vars2_h3.Out + K*0.5*Vars2_h5.Out + K*0.2*Vars2_h7.Out + K*0.2*Vars2_h9.Out + K*0.0*Vars2_h11.Out);
		//cntl_pi5.Ref =cntl_pi2.Out + 3800.0*Vars2_h1.Out;
	   	cntl_pi5.Fbk = I_S; 
	    CNTL_PI_F_FUNC(&cntl_pi5);   
    
//---------------------PI Regulator for Phase c Current--------------------------------------
    	cntl_pi6.Ref =cntl_pi3.Out + 1000.0*(Vars3_h1.Out + K*0.5*Vars3_h3.Out + K*0.5*Vars3_h5.Out + K*0.2*Vars3_h7.Out + K*0.2*Vars3_h9.Out + K*0.0*Vars3_h11.Out);;
...
void Reset_System(void){
	...
	Flag_Start=0,Counter_Start=0,Flag_Sync=2,Flag_ShutDown=0,Counter_Emergency_Shutdown=0,nsamples=0,Counter_SyncSlowly=0,Flag_PowerOk=0;
	...
	PowerGood=0, PowerBad=0, NormalPower_Counter=0, TurnOnBypass=0,Flag_ZCD=0,Counter_Sequence_OK=0, Counter_Sync_OK=0, GPIO58_prev=0,Flag_Manual_STS = 0, Flag_Manual_Bypass = 0, Flag_Auto_STS = 0;
